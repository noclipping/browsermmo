...
/** Leave the wilds and open the global player market in one step (blocked during active combat). */
export async function returnToTownAndMarketAction() {
  const user = await requireUser();
  const character = await requireCharacter(user.id);
  const inCombat = await prisma.soloCombatEncounter.findFirst({
    where: { characterId: character.id, status: "ACTIVE" },
  ...
  const keys = await prisma.$transaction(async (tx) => {
    ...
    if (slotRow?.itemId && slotRow.itemId !== invItem.itemId) {
      await returnGearToInventoryTx(tx, {
        ...
        bonusDefensePercent: slotRow.bonusDefensePercent,
        bonusConstitutionPercent: slotRow.bonusConstitutionPercent,
    ...
    await tx.characterEquipment.update({
      ...
      data: {
        ...
        bonusDefensePercent: samePiece ? (slotRow?.bonusDefensePercent ?? 0) : (invItem.bonusDefensePercent ?? 0),
        bonusConstitutionPercent:
          samePiece ? (slotRow?.bonusConstitutionPercent ?? 0) : (invItem.bonusConstitutionPercent ?? 0),
...
export async function unequipSlotAction(formData: FormData) {
  ...
  const keys = await prisma.$transaction(async (tx) => {
    await returnGearToInventoryTx(tx, {
      ...
      bonusDefensePercent: worn.bonusDefensePercent,
      bonusConstitutionPercent: worn.bonusConstitutionPercent,
