...
export default async function SocialRequestsPage() {
  const user = await requireUser();
  const character = await requireCharacter(user.id);

  const [equipment, inventory, currentRegion, townRegion, combatActive, incoming, outgoing] = await Promise.all([
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(16rem,1fr)_minmax(0,56rem)_minmax(16rem,1fr)] lg:items-start">
          ...
          <div className="min-w-0 space-y-4">
            ...
            <div className="rounded-2xl border border-white/20 bg-zinc-950/45 bg-linear-to-b from-black/62 via-black/78 to-black/95 p-5 shadow-md backdrop-blur-[1px]">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/60">Social · Requests</p>
              <h1 className="mt-1 font-serif text-2xl text-zinc-100">Incoming &amp; outgoing</h1>
            ...
            <section className="rounded-2xl border border-white/20 bg-zinc-950/45 bg-linear-to-b from-black/62 via-black/78 to-black/95 p-4 shadow-md backdrop-blur-[1px]">
              <h2 className="text-[10px] font-bold uppercase tracking-widest text-amber-500/90">Incoming requests</h2>
              {incoming.length === 0 ? (
            ...
            <p className="text-center text-sm text-zinc-500">
              <Link href="/social/directory" className="text-amber-200/90 underline decoration-amber-900/50 hover:text-amber-100">
                Player directory
              </Link>
              {" · "}
              <Link href="/social/friends" className="text-amber-200/90 underline decoration-amber-900/50 hover:text-amber-100">
                Friends list
              </Link>
            </p>
          </div>
