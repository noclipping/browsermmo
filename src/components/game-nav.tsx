...
export function GameNav({
  ...
  const inactive = "border-zinc-700 bg-zinc-900/80 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200";
  const activeCls = "border-amber-600 bg-amber-950/50 text-amber-100";

  const townBlockedTitle = "Finish or flee your current fight before traveling.";
  const townTravelTitle = inTownRegion ? "Town hub" : "Return to Town Outskirts (one step)";

  return (
    <nav className="mb-6 flex flex-wrap gap-2 border-b border-amber-900/25 pb-4" aria-label="Main">
      {pathname === "/" ? (
        ...
        <Link href="/" className={`${pill} ${tap} ${inactive}`} title="Town hub">
          Town
        </Link>
      ) : (
      ...
      {combatLocked ? (
        ...
        <Link href="/shop" className={`${pill} ${tap} ${inactive}`} title="Town market (Town Outskirts only)">
          Shop
        </Link>
      )}
